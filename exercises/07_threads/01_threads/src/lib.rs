...
pub fn sum(v: Vec<i32>) -> i32 {
    let (v1, v2) = v.split_at(v.len() / 2); //&v[..v.iter().len() / 2];
    let v1 = v1.to_vec();
    let v2 = v2.to_vec();
    let r1 = thread::spawn(move || { v1.iter().sum::<i32>() });
    let r2 = thread::spawn(move || { v2.iter().sum::<i32>() });
    if let Ok(rs1) = r1.join() {
        if let Ok(rs2) = r2.join() {
            return rs1 + rs2;
        }
    }
    0
